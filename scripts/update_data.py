...
def fetch(url):
    req = Request(url, headers={'User-Agent': 'Volcanic-Ash-Observation/1.4'})
...
def parse_vaac(html):
    ...
    return {
        'advisory': adv, 'dtg': dtg, 'status': 'verified', 'volcano': 'KRAKATAU',
        'lower_level': 'SFC/FL150', 'lower_direction': low.group(1).upper() if low else None,
        'higher_level': 'SFC/FL500', 'higher_direction': high.group(1).upper() if high else None,
        'low_level_polygons': polygons, 'raw_summary': block[:3000],
    }
